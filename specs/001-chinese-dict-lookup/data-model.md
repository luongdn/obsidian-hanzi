...
```
[Unloaded] → onload() → [Registered: extensions + commands, no lookups yet]
[Registered] → onLayoutReady() → [Loading Dictionary]
...
[Parsing] → parse complete → refreshExtensions() + refreshReadingMode() → [Ready]
